...
func TestRateWriterSplitsWritesLargerThanBurst(t *testing.T) {
	underlying := &testBufferWriter{}
	bucket := rate.NewLimiter(100, 4)
	writer := New().RateWriterContext(context.Background(), underlying, bucket)
