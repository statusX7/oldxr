...
mkdir -p "${release_root}/v0.9.0-r1" "${raw_root}" "${install_root}"
...
grep -F "maintenance channel 0.9.0 当前解析为 v0.9.0-r1" <<<"${fresh_output}" >/dev/null
grep -F "oldxr v0.9.0-r1 安装完成" <<<"${fresh_output}" >/dev/null
...
"${install_root}/usr/local/XrayR/XrayR" -version | grep -F "XrayR 0.9.0-r1" >/dev/null
...
echo "执行 update test"
update_output="$(run_installer 0.9.0-r1)"
grep -F "使用显式 maintenance release：v0.9.0-r1" <<<"${update_output}" >/dev/null
...
grep -F "maintenance channel 0.9.0 当前解析为 v0.9.0-r1" <<<"${manager_output}" >/dev/null
...
printf 'corrupt' >> "${release_root}/v0.9.0-r1/${archive_name}"
