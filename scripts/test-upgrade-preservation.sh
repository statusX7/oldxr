...
if [[ $# -lt 3 || $# -gt 4 ]]; then
    echo "用法：$0 RELEASE_ARCHIVE INSTALL_SCRIPT OFFICIAL_V0_9_0_BINARY [OLDXR_V1_0_2_BINARY]" >&2
    exit 2
fi
...
if [[ -n "${oldxr_binary}" ]]; then
    [[ -f "${oldxr_binary}" ]] || { echo "错误：oldxr v1.0.2 binary不存在：${oldxr_binary}" >&2; exit 1; }
    "${oldxr_binary}" -version | grep -F 'XrayR 1.0.2' >/dev/null || {
        echo "错误：输入 binary 不是 exact public oldxr v1.0.2。" >&2
        exit 1
    }
fi
...
prepare_oldxr_layout() {
    ...
    chmod +x "${root}/usr/local/XrayR/XrayR"
    printf '%s\n' 'v1.0.2' > "${root}/usr/local/XrayR/.oldxr-release"
    cat > "${root}/usr/bin/XrayR" <<'MANAGER'
#!/usr/bin/env bash
...
if [[ -n "${oldxr_binary}" ]]; then
    echo "执行 exact public oldxr v1.0.2 非默认配置路径升级"
    prepare_oldxr_layout oldxr-v102 space
    assert_successful_upgrade oldxr-v102 oldxr
fi
...
if [[ -n "${oldxr_binary}" ]]; then
    echo "PASS：官方 XrayR v0.9.0 三种 config flag 与 oldxr v1.0.2 均无损升级，用户文件元数据、永久备份及启动失败自动回滚通过。"
else
