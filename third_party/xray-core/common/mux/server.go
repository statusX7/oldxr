...
// DispatchDirect forwards non-mux TCP requests to dispatchers that support
// direct socket ownership. Mux sessions must keep using the framed pipe path.
func (s *Server) DispatchDirect(ctx context.Context, dest net.Destination, input buf.Reader) (*transport.DirectLink, buf.Reader, error) {
	if dest.Address == muxCoolAddress {
		return nil, input, nil
	}
	direct, ok := s.dispatcher.(routing.DirectDispatcher)
	if !ok {
		return nil, input, nil
	}
	return direct.DispatchDirect(ctx, dest, input)
}
