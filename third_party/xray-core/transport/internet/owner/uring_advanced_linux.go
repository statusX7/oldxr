...
type advancedUringOwnerDeferredSend struct {
	sessionID uint64
	side      uint8
}
...
type advancedUringOwnerLoop struct {
	...
	probeEpoch    uint64
	probeUsed     bool
	deferredSends []advancedUringOwnerDeferredSend
	deferredSpare []advancedUringOwnerDeferredSend
...
func (loop *advancedUringOwnerLoop) runLocked() {
	...
	for !loop.stopping.Load() {
		// A writable callback may consume data that was already buffered by a
		// multishot receive and queue another completed direct send. Give that
		// next logical generation a scheduler turn before acknowledging it; the
		// common paced path never enters this branch.
		if len(loop.deferredSends) != 0 {
			runtime.Gosched()
		...
		for {
			...
			// Direct sends that completed synchronously remain logically pending
			// until every CQE already visible in this batch has had a chance to
			// run. This preserves the existing protocol pause/resume boundary
			// without allocating one kernel send CQE (or a synthetic NOP CQE) per
			// record.
			loop.completeDeferredSends()
...
func (loop *advancedUringOwnerLoop) queueDeferredSend(session *advancedUringOwnerSession, side, written int) error {
	endpoint := &session.endpoint[side]
	if endpoint.sendPending || endpoint.sendCompletion || endpoint.sendDeferred || session.closing {
		return errors.New("owner: advanced io_uring deferred send state is busy")
	}
	if written <= 0 {
		return errors.New("owner: advanced io_uring deferred send made no progress")
	}
	endpoint.sendPending = true
	endpoint.sendDeferred = true
	endpoint.sendN = written
	endpoint.sendErr = nil
	loop.deferredSends = append(loop.deferredSends, advancedUringOwnerDeferredSend{
...
// completeDeferredSends publishes exactly one logical generation. Callbacks
// that synchronously consume an already-buffered record append to the swapped
// queue and are therefore acknowledged on a later loop turn, never recursively
// in the same generation.
func (loop *advancedUringOwnerLoop) completeDeferredSends() {
	if len(loop.deferredSends) == 0 {
		return
	}
	pending := loop.deferredSends
	loop.deferredSends = loop.deferredSpare[:0]
	for _, deferred := range pending {
		if deferred.sessionID == 0 || deferred.sessionID >= uint64(len(loop.sessions)) {
			continue
		}
		session := loop.sessions[deferred.sessionID]
		if session == nil || session.closed {
			continue
		}
		side := int(deferred.side)
		if side < 0 || side >= len(session.endpoint) {
			continue
		}
		endpoint := &session.endpoint[side]
		if !endpoint.sendDeferred {
			continue
		}
...
func (conn *advancedUringOwnerConn) TryWrite(payload []byte) (int, error) {
	...
	if written == len(payload) {
		if deferErr := conn.loop.queueDeferredSend(conn.session, conn.side, written); deferErr != nil {
			// The bytes are already in the socket. Return exact progress so the
			// protocol never retransmits them if logical acknowledgement cannot
			// be queued.
			return written, deferErr
		}
		return 0, nil
	}
