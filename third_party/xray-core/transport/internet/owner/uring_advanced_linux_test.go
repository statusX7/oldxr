...
type advancedUringOwnerDeferredSession struct {
	writable []Role
	hook     func()
}

func (*advancedUringOwnerDeferredSession) OnOpen(Role, Conn) {}

func (*advancedUringOwnerDeferredSession) OnTraffic(Role, Conn) Action {
	return None
}

func (session *advancedUringOwnerDeferredSession) OnWritable(role Role, _ Conn) Action {
	session.writable = append(session.writable, role)
	if session.hook != nil {
		session.hook()
	}
	return None
...
func (*advancedUringOwnerDeferredSession) OnReadClosed(Role, Conn) Action {
	return None
}

func (*advancedUringOwnerDeferredSession) OnClose(Role, Conn, error) {}
...
func newAdvancedUringOwnerDeferredFixture(loop *advancedUringOwnerLoop, id uint64, side int, handler Session) (*advancedUringOwnerSession, *advancedUringOwnerConn) {
	session := &advancedUringOwnerSession{id: id, handler: handler}
...
func TestAdvancedUringOwnerDefersDirectSendAcknowledgement(t *testing.T) {
	...
	if err := loop.queueDeferredSend(session, 0, 4096); err != nil {
		t.Fatal(err)
	}
	endpoint := &session.endpoint[0]
	if !endpoint.sendPending || !endpoint.sendDeferred || endpoint.sendCompletion {
		t.Fatalf("queued state = {pending:%v deferred:%v completion:%v}", endpoint.sendPending, endpoint.sendDeferred, endpoint.sendCompletion)
	}
	if len(handler.writable) != 0 {
		t.Fatalf("writable callbacks before batch end = %v", handler.writable)
	}

	loop.completeDeferredSends()

	if endpoint.sendPending || endpoint.sendDeferred || !endpoint.sendCompletion {
		t.Fatalf("completed state = {pending:%v deferred:%v completion:%v}", endpoint.sendPending, endpoint.sendDeferred, endpoint.sendCompletion)
	}
	if len(handler.writable) != 1 || handler.writable[0] != Inbound {
		t.Fatalf("writable callbacks = %v; want [%v]", handler.writable, Inbound)
	}
	written, err := conn.TryWrite(make([]byte, 4096))
	if err != nil || written != 4096 {
		t.Fatalf("logical completion = (%d, %v); want (4096, nil)", written, err)
	}
}

func TestAdvancedUringOwnerDefersReentrantGeneration(t *testing.T) {
	loop := &advancedUringOwnerLoop{sessions: make([]*advancedUringOwnerSession, 3)}
	firstHandler := &advancedUringOwnerDeferredSession{}
	secondHandler := &advancedUringOwnerDeferredSession{}
	first, _ := newAdvancedUringOwnerDeferredFixture(loop, 1, 0, firstHandler)
	second, _ := newAdvancedUringOwnerDeferredFixture(loop, 2, 1, secondHandler)
	firstHandler.hook = func() {
		if err := loop.queueDeferredSend(second, 1, 2048); err != nil {
			t.Errorf("queue second generation: %v", err)
		}
	}

	if err := loop.queueDeferredSend(first, 0, 1024); err != nil {
		t.Fatal(err)
	}
	loop.completeDeferredSends()

	if len(firstHandler.writable) != 1 {
		t.Fatalf("first generation callbacks = %v", firstHandler.writable)
	}
	if len(secondHandler.writable) != 0 {
		t.Fatalf("second generation completed recursively: %v", secondHandler.writable)
	}
	if len(loop.deferredSends) != 1 {
		t.Fatalf("queued next generation = %d; want 1", len(loop.deferredSends))
	}

	loop.completeDeferredSends()
	if len(secondHandler.writable) != 1 || secondHandler.writable[0] != Outbound {
		t.Fatalf("second generation callbacks = %v; want [%v]", secondHandler.writable, Outbound)
	}
}

func TestAdvancedUringOwnerDropsDeferredCallbackWhileClosing(t *testing.T) {
	loop := &advancedUringOwnerLoop{sessions: make([]*advancedUringOwnerSession, 2)}
	handler := &advancedUringOwnerDeferredSession{}
	session, _ := newAdvancedUringOwnerDeferredFixture(loop, 1, 0, handler)
	if err := loop.queueDeferredSend(session, 0, 1024); err != nil {
		t.Fatal(err)
	}
	// Keep a receive outstanding so the synthetic fixture does not require a
	// live ring when maybeCloseSession evaluates the normal close barrier.
	...
	endpoint := &session.endpoint[0]
	if endpoint.sendPending || endpoint.sendDeferred || endpoint.sendCompletion {
		t.Fatalf("closing state = {pending:%v deferred:%v completion:%v}", endpoint.sendPending, endpoint.sendDeferred, endpoint.sendCompletion)
	}
	if len(handler.writable) != 0 {
		t.Fatalf("closing session received writable callback: %v", handler.writable)
	}
}
...
func TestOwnerHybridReactorRelaysThroughBothBackends(t *testing.T) {
	...
	if _, err := advancedUringOwnerClient(); err != nil {
		t.Skipf("advanced io_uring unavailable; hybrid backend split cannot be exercised: %v", err)
	}
