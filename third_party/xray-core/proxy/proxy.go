...
// DirectOutbound optionally exposes a directly owned outbound connection.
// Implementations must preserve the same dialing, policy and accounting
// semantics as Process.
type DirectOutbound interface {
	OpenDirect(context.Context, net.Destination, internet.Dialer) (*transport.DirectLink, error)
}
