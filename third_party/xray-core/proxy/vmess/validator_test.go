...
func TestAEADUserValidatorRejectsLegacyHash(t *testing.T) {
	hasher := protocol.DefaultIDHash
	v := NewAEADUserValidator(hasher)
	defer common.Close(v)
	...
	idHash := hasher(id.Bytes())
	common.Must2(serial.WriteUint64(idHash, uint64(time.Now().Unix())))
	legacyHash := idHash.Sum(nil)
	if got, _, found, err := v.Get(legacyHash); found || got != nil || err != ErrNotFound {
		t.Fatalf("legacy hash unexpectedly accepted: user=%v found=%v err=%v", got, found, err)
	}
}
