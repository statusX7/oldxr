...
func New(ctx context.Context, config *Config) (*Handler, error) {
	...
	var clients *vmess.TimedUserValidator
	if aeadForced {
		clients = vmess.NewAEADUserValidator(protocol.DefaultIDHash)
	} else {
...
func (h *Handler) Process(ctx context.Context, network net.Network, connection stat.Connection, dispatcher routing.Dispatcher) error {
	...
	ownerInbound, ownerInboundReads, ownerInboundWrites, ownerCandidate := vmessOwnerConnection(connection, dispatcher)
	var ownerCodec *encoding.OwnerBodyCodec
	var bodyReader buf.Reader
	if ownerCandidate {
		ownerCodec, err = svrSession.NewOwnerBodyCodec(request, reader)
		if err == nil {
			bodyReader = ownerCodec
			ownerVMessAttempts.Add(1)
		} else {
			ownerCodec = nil
		}
	}
	...
	if request.Command == protocol.RequestCommandTCP {
		if directDispatcher, ok := dispatcher.(routing.DirectDispatcher); ok {
			if err := connection.SetReadDeadline(time.Now().Add(200 * time.Millisecond)); err == nil {
				directLink, replayReader, directErr := directDispatcher.DispatchDirect(ctx, request.Destination(), bodyReader)
				if replayReader != nil {
				...
				if directErr != nil {
					return newError("failed to dispatch direct request to ", request.Destination()).Base(directErr)
				}
				if directLink != nil {
					flow := directLink.Flow()
					if ownerCodec != nil && directLink.Connection != nil && flow != nil && flow.OwnerEligible() {
						pending, buffered := takeVMessOwnerPlaintext(bodyReader, ownerCodec)
						if buffered {
							response := &protocol.ResponseHeader{Command: h.generateCommand(ctx, request)}
							if ownerErr := ownerCodec.PrepareResponse(response); ownerErr != nil {
								_ = directLink.Close()
								cancel()
								return newError("failed to prepare VMess owner response").Base(ownerErr)
							}
							flow = directLink.TakeFlow()
							ownerSession := newOwnerVMessSession(ownerCodec, directLink, flow, sessionPolicy.Timeouts, pending, ownerCodec.TakeEncryptedBuffered(), ownerInboundReads, ownerInboundWrites)
							if ownerErr := owner.AdoptPair(ownerInbound, directLink.Connection, ownerSession); ownerErr != nil {
								flow.Release()
								ownerSession.flow = nil
								_ = directLink.Close()
								cancel()
								return newError("failed to transfer VMess sockets to owner reactor").Base(ownerErr)
							}
							ownerVMessSuccess.Add(1)
							_ = directLink.Close()
							cancel()
							return nil
						}
	...
	responseDone := func() error {
		...
		return transferResponse(timer, svrSession, request, response, responseReader, writer)
	}
