...
func NewTimedUserValidator(hasher protocol.IDHash) *TimedUserValidator {
	return newTimedUserValidator(hasher, true)
}

// NewAEADUserValidator creates a validator without legacy timestamp hashes.
// It must only be used when the inbound rejects all non-AEAD requests.
func NewAEADUserValidator(hasher protocol.IDHash) *TimedUserValidator {
	return newTimedUserValidator(hasher, false)
}

func newTimedUserValidator(hasher protocol.IDHash, legacy bool) *TimedUserValidator {
	var userHash map[[16]byte]indexTimePair
	if legacy {
