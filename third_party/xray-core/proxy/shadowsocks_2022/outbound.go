...
func (o *Outbound) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	if network == net.Network_TCP {
		...
		if o.uot {
			serverConn := o.method.DialEarlyConn(connection, uot.RequestDestination(uot.LegacyVersion))
			client := &uot.Client{Version: uot.LegacyVersion}
			uotConn, err := client.DialEarlyConn(serverConn, false, toSocksaddr(destination))
			if err != nil {
				return newError("create UDP-over-TCP client").Base(err)
			}
