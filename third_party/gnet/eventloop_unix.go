...
func (el *eventloop) readClosed(c *conn) error {
	if c.readClosed {
		return nil
	}
	handler, ok := el.eventHandler.(ReadClosedEventHandler)
	if !ok {
	...
	if c.pollRegistered {
		if err := el.poller.Delete(c.fd); err != nil {
			return el.close(c, os.NewSyscallError("delete read-closed", err))
		}
	...
	action := handler.OnReadClosed(c)
	if err := el.handleAction(c, action); err != nil || !c.opened {
		return err
	}
	if !c.outboundBuffer.IsEmpty() || c.ownerWriteArmed {
