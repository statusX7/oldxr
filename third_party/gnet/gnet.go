...
type (
	...
	// ReadClosedEventHandler is an optional extension for stream-oriented
	// protocols that need TCP half-close semantics. EventHandler implementations
	// that don't implement it retain the original gnet behavior: a read EOF
	// closes the whole connection.
	ReadClosedEventHandler interface {
		OnReadClosed(c Conn) (action Action)
	}

	// WritableEventHandler is an optional extension used by protocols that keep
	// their own bounded output queue. It fires only after the protocol has armed
	// writable interest on a connection; ordinary gnet writes are unaffected.
	WritableEventHandler interface {
		OnWritable(c Conn) (action Action)
	}
