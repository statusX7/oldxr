...
type conn struct {
	...
	readClosed      bool                   // whether the remote has closed its writing half
	pollRegistered  bool                   // whether this connection is registered in the poller
	readPaused      bool                   // whether an owner protocol has suspended readable events
	ownerWriteArmed bool                   // whether an owner protocol is waiting for writable events
	ownerWriteOnly  bool                   // whether the poller currently watches writes only
}
...
func (c *conn) release() {
	...
	c.readPaused = false
	c.ownerWriteArmed = false
	c.ownerWriteOnly = false
	c.ctx = nil
	c.safeCtx.Store(nil)
	c.buffer = nil
...
func (c *conn) enableWritePoll() error {
	isET := c.loop.engine.opts.EdgeTriggeredIO
	if c.readClosed || c.readPaused {
		if c.pollRegistered && c.ownerWriteOnly {
			return nil
		}
		if c.pollRegistered {
			if err := c.loop.poller.Delete(c.fd); err != nil {
				return err
			}
			c.pollRegistered = false
		}
		if err := c.loop.poller.AddWrite(&c.pollAttachment, isET); err != nil {
			return err
		}
		c.pollRegistered = true
		c.ownerWriteOnly = true
		return nil
	}
	if !c.pollRegistered {
		if err := c.loop.poller.AddReadWrite(&c.pollAttachment, isET); err != nil {
			return err
		}
		c.pollRegistered = true
	} else if err := c.loop.poller.ModReadWrite(&c.pollAttachment, isET); err != nil {
		return err
	}
	c.ownerWriteOnly = false
	return nil
...
// TryWrite performs one nonblocking socket write without adding bytes to
// gnet's elastic output buffer. It is intentionally exposed only through the
...
// SuspendRead removes readable interest while preserving any required write
// interest. It must be called from the connection's event loop.
func (c *conn) SuspendRead() error {
	if c.readClosed || c.readPaused {
		return nil
	}
	c.readPaused = true
	if c.ownerWriteArmed || !c.outboundBuffer.IsEmpty() {
		return c.enableWritePoll()
	}
	if c.pollRegistered {
		if err := c.loop.poller.Delete(c.fd); err != nil {
			return err
...
// ResumeRead restores readable interest. It must be called from the
...
func (c *conn) ResumeRead() error {
	...
	c.readPaused = false
	isET := c.loop.engine.opts.EdgeTriggeredIO
	wantWrite := c.ownerWriteArmed || !c.outboundBuffer.IsEmpty()
	if c.pollRegistered {
		if wantWrite {
			if err := c.loop.poller.ModReadWrite(&c.pollAttachment, isET); err != nil {
				return err
			}
		} else if err := c.loop.poller.ModRead(&c.pollAttachment, isET); err != nil {
			return err
		}
	} else {
		add := c.loop.poller.AddRead
		if wantWrite {
			add = c.loop.poller.AddReadWrite
		}
		if err := add(&c.pollAttachment, isET); err != nil {
			return err
		}
		c.pollRegistered = true
	}
...
// ArmWrite enables optional WritableEventHandler callbacks until DisarmWrite
// is called. It must be called from the connection's event loop.
func (c *conn) ArmWrite() error {
	if c.ownerWriteArmed {
		return nil
	}
	c.ownerWriteArmed = true
	return c.enableWritePoll()
}

// DisarmWrite removes owner writable interest while retaining gnet's own
// buffered-write and readable interests.
func (c *conn) DisarmWrite() error {
	if !c.ownerWriteArmed {
		return nil
	}
	c.ownerWriteArmed = false
	...
	isET := c.loop.engine.opts.EdgeTriggeredIO
	if !c.pollRegistered {
		if err := c.loop.poller.AddRead(&c.pollAttachment, isET); err != nil {
			return err
		}
		c.pollRegistered = true
	} else if err := c.loop.poller.ModRead(&c.pollAttachment, isET); err != nil {
...
loop:
	if sent, err = unix.Write(c.fd, data); err != nil {
		...
		if err == unix.EAGAIN {
			...
			if !isET || c.readClosed {
				err = c.enableWritePoll()
			}
			return
		}
		return 0, err
