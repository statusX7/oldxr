...
- 任一CPU、RAM、功能、OS、soak、安全、升级或rollback Gate失败时，不得发布`v1.0.x`、不得更新master安装入口、不得宣称完成。用户明确接受的特定风险只能记录在对应版本的本地内部报告中，不能通过公开披露或模糊表述把内部FAIL改写成PASS；公开Release notes仍只写高层维护结论。
