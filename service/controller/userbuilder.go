...
func (c *Controller) buildVmessUser(userInfo *[]api.UserInfo, serverAlterID uint16, tag string) (users []*protocol.User) {
	...
	for i, user := range *userInfo {
		...
		users[i] = &protocol.User{
			Level:   0,
			Email:   c.buildUserTagWithTag(tag, &user), // Email: InboundTag|email|uid
...
func (c *Controller) buildSSUser(userInfo *[]api.UserInfo, method, tag string) (users []*protocol.User) {
	users = make([]*protocol.User, len(*userInfo))
	...
	for i, user := range *userInfo {
		...
		if C.Contains(shadowaead_2022.List, strings.ToLower(method)) {
			e := c.buildUserTagWithTag(tag, &user)
...
func (c *Controller) buildUserTagWithTag(tag string, user *api.UserInfo) string {
